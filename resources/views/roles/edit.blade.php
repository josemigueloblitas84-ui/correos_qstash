                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('roles.update', $role->id) }}" method="POST">
                        ...
                        <div>
                            ...
                            <div class="my-3">
                                <input value="{{ old('name', $role->name) }}" name="name" id="name" type="text"
                            ...
                            <div class="grid grid-cols-4 my-5">
                                @if ($permisos->isNotEmpty())
                                    @foreach ($permisos as $permiso)
                                        <div class="mt-3">
                                            <input {{ $hasPermisos->contains($permiso->name) ? 'checked' : '' }} type="checkbox" id="permiso-{{ $permiso->id }}" name="permisos[]" value="{{ $permiso->name }}"
                                                class="rounded">
