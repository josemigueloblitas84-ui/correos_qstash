        ...
        return DataTables::of($query)
            ...
            ->addColumn('fechas_agendadas', function ($row) {
                $desde = $row->fecha_desde
                    ? date('d-m-Y', strtotime($row->fecha_desde))
                    : '';

                $hasta = $row->fecha_hasta
                    ? date('d-m-Y', strtotime($row->fecha_hasta))
                    : '';
    ...
    public function dataInforme(Request $request)
    {
        ...
        return DataTables::of($query)
            ...
            ->addColumn('visualizar', function ($row) {
                return '<button type="button" class="btn-accion-imprimir" title="Visualizar">
                            <i class="fas fa-print"></i>
                        </button>';
            })
            ->addColumn('validar', function ($row) {
                if ((int) $row->validada_encargado === 1) {
                    return '<span class="estado-validada">Validada</span>';
                }

                return '<button type="button" class="btn-validar" title="Validar">V</button>';
            })
        ...
        return DataTables::of($query)
            ...
            ->addColumn('visualizar', function ($row) {
                return '<button
                            ...
                            data-departamento="' . e($row->departamento_id) . '"
                            title="Visualizar">
                            <i class="fas fa-print"></i>
                        </button>';
            })
