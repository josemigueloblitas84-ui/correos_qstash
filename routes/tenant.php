...
use App\Http\Controllers\Auth\TenantCentralLoginController;
...
|--------------------------------------------------------------------------
|
| These routes run inside an institution tenant. The domain identifies the
| institution, and the active database is switched before these routes run.
|
*/
...
Route::middleware([
    ...
    Route::post('/central-login/disconnect', [TenantCentralLoginController::class, 'disconnect'])
        ->middleware('auth')
        ->name('tenant.central-login.disconnect');
